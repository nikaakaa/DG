...
- 将 `QueryAutoMove`、`QueryPushOnEnter` 和真实规则热路径组件组合查询下沉到 Arch backend 能力，而不是回退为全实体扫描。
...
- 用户手动验证：启动 Fantasy 服务端，Unity Play Mode 启动两个客户端，覆盖 JoinWorld、玩家移动、自动移动、机关推动、connected body、debug spawn/move/remove、runtime effect，并确认两个客户端只跟随同一服务端 `WorldDelta` 序列。
