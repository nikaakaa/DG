...
系统 SHALL 在客户端进入共享世界时由服务端分配唯一 player entity，并把该 entity 注册进服务端权威移动世界。

#### Scenario: 第一个客户端进入世界
- **WHEN** 一个已连接 session 发送 JoinWorld 请求
- **THEN** 服务端分配一个唯一 player entity id
- **AND** 服务端把该 entity 注册到权威移动世界
...
### Requirement: 多玩家占位规则
系统 SHALL 在服务端权威移动世界中阻止不同 player entity 占用同一个格子。
...
系统 SHALL 在 JoinWorld 成功后把 session 加入共享世界 observer 集合，使客户端无需手动声明观察某个 entity 才能接收多人同步。

#### Scenario: 新客户端 Join 后接收已有玩家
- **WHEN** 客户端 B Join 成功
- **AND** 客户端 A 的 player entity 已存在
- **THEN** 客户端 B 能收到 A 的当前 player entity 坐标 snapshot 或等价通知

#### Scenario: 已有客户端收到新玩家
- **WHEN** 客户端 C Join 成功
- **AND** 客户端 A 和 B 已在线
- **THEN** 客户端 A 和 B 能收到 C 的 player entity 出现或坐标 snapshot
