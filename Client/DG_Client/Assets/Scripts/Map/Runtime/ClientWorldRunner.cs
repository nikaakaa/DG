...
namespace DG.Map
{
    public sealed class ClientWorldRunner : MonoBehaviour
    {
        ...
        public void SubmitMovement(long entityId, Vector2Int targetCoord)
        {
            if (Context == null)
            {
                return;
            ...
            Context.EnqueueMoveCommand(MoveCommand.ToTarget(
                entityId,
                MapCoordinate.ToGridCoord(targetCoord),
                MoveCommandSource.PlayerInput,
                Context.TickIndex,
                Context.TickIndex));
        }
