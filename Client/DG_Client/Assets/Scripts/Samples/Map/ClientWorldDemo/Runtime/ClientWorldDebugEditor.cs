...
namespace DG.Map
{
    ...
    public sealed class ClientWorldDebugEditor : MonoBehaviour
    {
        ...
        private void OnGUI()
        {
            ...
            DrawSlot(startX + slotWidth * 3, y, slotWidth, slotHeight, DebugWorldEditorSlot.Drag, "4 Drag");
            DrawSlot(startX + slotWidth * 4, y, slotWidth, slotHeight, DebugWorldEditorSlot.Delete, "5 Delete");

            GUI.Label(new Rect(18, Screen.height - 74, 520, 24), $"Coord: {(hasHover ? hoveredCoord.ToString() : "-")}  Direction: {buildDirection}  Selected: {selectedEntityId}");
        ...
        private void ExecuteCurrentTool()
        {
            ...
            if (IsBuildSlot(currentSlot))
            {
                ...
                int interval = currentSlot == DebugWorldEditorSlot.Ball ? 1 : 1;
            ...
            if (currentSlot == DebugWorldEditorSlot.Drag)
            {
                if (!dragging)
                {
                    dragging = true;
        ...
        {
            return slot == DebugWorldEditorSlot.Blocker || slot == DebugWorldEditorSlot.Ball || slot == DebugWorldEditorSlot.Conveyor;
        }
