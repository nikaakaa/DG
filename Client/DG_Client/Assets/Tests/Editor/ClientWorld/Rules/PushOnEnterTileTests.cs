...
namespace DG.EditorTests
{
    public sealed class PushOnEnterTileTests
    {
        ...
        [Test]
        public void SnapshotComponentRegistry_AddsProjectorWithoutGameWorldSnapshotBranch()
        {
            var registry = new SnapshotComponentRegistry();
            registry.Register(new TestSnapshotProjection(), new TestSnapshotProjection());
            var world = new GameWorld();
            Assert.IsTrue(world.AddEntity(DefaultWorldConfig.BlockerSpawn(501, new GridCoord(3, 4))));
            Assert.IsTrue(world.TryGetEntity(501, out GameEntity entity));

            EntitySnapshot snapshot = registry.Project(world, entity, 9);

            Assert.AreEqual(1234, snapshot.X);
            Assert.AreEqual(9, snapshot.ServerTick);
        }
