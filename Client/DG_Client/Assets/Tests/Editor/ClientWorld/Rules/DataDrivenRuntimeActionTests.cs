...
namespace DG.EditorTests
{
    public sealed class DataDrivenRuntimeActionTests
    {
        ...
        public void GameWorldStorageAdapterBoundary_PreventsInternalStorageLeakage()
        {
            ...
            string[] ruleFiles = Directory.GetFiles(Path.Combine(root, "Shared", "DG.GameCore", "ActionRuntime"), "*.cs", SearchOption.AllDirectories);
            ...
            string[] sharedFiles = Directory.GetFiles(Path.Combine(root, "Shared", "DG.GameCore"), "*.cs", SearchOption.AllDirectories)
                .Where(path => !path.EndsWith("ArchWorldDataStorage.cs", StringComparison.Ordinal))
                .ToArray();
        ...
        public void CoreExecutionAndArbitration_DoNotBranchOnOrdinaryActionKinds()
        {
            ...
            {
                File.ReadAllText(Path.Combine(root, "Shared", "DG.GameCore", "ActionRuntime", "Claims", "ActionArbiter.cs")),
                File.ReadAllText(Path.Combine(root, "Shared", "DG.GameCore", "ActionRuntime", "Claims", "ActionClaims.cs")),
                File.ReadAllText(Path.Combine(root, "Shared", "DG.GameCore", "ActionRuntime", "Requests", "ActionRequests.cs")),
                File.ReadAllText(Path.Combine(root, "Shared", "DG.GameCore", "ActionRuntime", "Specs", "ActionSpecRegistry.cs")),
                File.ReadAllText(Path.Combine(root, "Shared", "DG.GameCore", "ActionRuntime", "Blocking", "Policies", "BlockedResultResolver.cs"))
            });
            string pendingPath = Path.Combine(root, "Shared", "DG.GameCore", "ActionRuntime", "Pending", "PendingRuleStates.cs");
        ...
        public void RuleModules_DoNotContainOrdinaryActionNameBranches()
        {
            ...
            string[] ruleFiles = new[]
            {
                ...
                Path.Combine(root, "Shared", "DG.GameCore", "ActionRuntime", "Planning", "RulePlanning.cs"),
            };
