...
public readonly struct ActionStrategyContext
{
    ...
    public ActionStrategyContext(GameWorld world, ActionRequest request, ActionSpec spec, List<CommitProposal> proposals, List<ActionRequest> moveRequests, Dictionary<long, MoveResult> actionResults, List<string> reasons, long serverTick, IGameConfigProvider? configProvider)
        : this(world, request, spec, proposals, moveRequests, actionResults, reasons, serverTick, configProvider, null)
    {
    }

    public ActionStrategyContext(GameWorld world, ActionRequest request, ActionSpec spec, List<CommitProposal> proposals, List<ActionRequest> moveRequests, Dictionary<long, MoveResult> actionResults, List<string> reasons, long serverTick, IGameConfigProvider? configProvider, IEnumerable<TargetFilterSpec>? targetFilters)
    {
        World = world;
        Request = request;
        Spec = spec;
