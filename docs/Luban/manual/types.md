...
| 类型 | 描述 |
|------|------|
| bool | bool类型，`true、false、0、1`都能被识别，大小写不敏感 |
...
| double | 对应c#的double |
| string | 对应c#的string |
| text | 语法糖类型，等价于`string#text=1`，校验本地化key合法性 |
...
|------|------|
| enum | 枚举类，对应c#的enum |
| bean | 复合类型，对应c#的class/struct，支持类型继承和多态 |
...
| map | 对应c#的Dictionary，定义方式为`map,<keyType>,<valueType>`。keyType只能为基本类型或enum类型，key与value都不能为可空类型 |
...
- **table**：为大多数语言生成代码时会为每个table生成一个类，管理这个表的所有数据。
- **tables**：为大多数语言生成代码时会包含一个所有表的管理类，类名在`luban.conf`中的`targets[xxx].manager`字段定义，一般取名为Tables。
